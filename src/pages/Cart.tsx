...
export function Cart() {
  ...
  const shipping = subtotal > 500 ? 0 : 45.0;
      ...
      <div className="pt-40 pb-24 px-6 max-w-7xl mx-auto text-center space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-zinc-500 max-w-md mx-auto">
            Parece que você ainda não adicionou nenhum item de luxo ao seu
            carrinho. Explore nossa coleção exclusiva!
          </p>
        </div>
  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-12">
        ...
        <Link
          to="/catalog"
          className="flex items-center gap-2 text-sm text-zinc-500 hover:text-accent transition-colors"
        >
          ...
          <AnimatePresence mode="popLayout">
            {cart.map((item) => (
              ...
              >
                <Link
                  to={`/product/${item.id}`}
                  className="w-24 h-24 rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 flex-shrink-0"
                >
                  <img
                    src={item.images[0]}
                    alt={item.name}
                ...
                <div className="text-right min-w-[120px]">
                  ...
                  <p className="text-xs text-zinc-500">
                    {item.quantity}x R${' '}
                    {(item.promoPrice || item.price).toLocaleString('pt-BR', {
        ...
        <div className="space-y-6">
          <div className="glass-card p-8 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {shipping > 0 && (
                <p className="text-[10px] text-zinc-500 text-right italic">
                  Faltam R$ {(500 - subtotal).toLocaleString('pt-BR')} para frete
                  grátis
                </p>
              )}
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                ...
                />
                <button className="btn-outline py-2 px-6 text-xs uppercase tracking-widest font-bold">
                  Aplicar
                </button>
              </div>
          ...
          <div className="glass-card p-6 bg-accent/5 border-accent/20">
            <p className="text-xs text-center text-zinc-400">
              Precisa de ajuda? Fale com um consultor via{' '}
