...
import { Filter, ChevronDown, LayoutGrid, List, Search, X, ChevronRight, Loader2, AlertCircle } from 'lucide-react';
...
export function Catalog() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (sortBy) {
      case 'price-low':
        result.sort((a, b) => ((a?.promoPrice || a?.price) || 0) - ((b?.promoPrice || b?.price) || 0));
        break;
      case 'price-high':
        result.sort((a, b) => ((b?.promoPrice || b?.price) || 0) - ((a?.promoPrice || a?.price) || 0));
        break;
      case 'rating':
        ...
        <div className="flex-grow space-y-8">
          {isLoading && products.length === 0 ? (
            <div className="py-24 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-zinc-500 font-display uppercase tracking-widest text-sm">Atualizando Catálogo...</p>
            </div>
          ) : error ? (
            <div className="py-24 flex flex-col items-center justify-center space-y-6 text-center">
              ...
              <div className="space-y-2">
                <h2 className="text-2xl font-display font-bold">Erro ao carregar catálogo</h2>
                <p className="text-zinc-500 max-w-md">Não conseguimos conectar à nossa base de produtos.</p>
              </div>
