...
export function AdminSidebar() {
  ...
  const menuItems = [
    ...
    { id: 'products', label: 'Produtos', icon: Package, path: '/admin/produtos' },
    { id: 'orders', label: 'Pedidos', icon: ShoppingBag, path: '/admin/pedidos' },
    { id: 'users', label: 'Usuários', icon: Users, path: '/admin/usuarios' },
  ...
  return (
    <aside className="w-64 h-screen bg-luxury-black border-r border-zinc-800 fixed left-0 top-0 z-50 flex flex-col">
      ...
      <nav className="flex-grow p-4 space-y-2 overflow-y-auto">
        <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 px-4 mb-4">Menu Principal</div>
        {menuItems.map((item) => {
