...
export function ProductList() {
  const { products, isLoading, error, fetchProducts, deleteProduct, toggleFeatured } = useProductStore();
  ...
  const filteredProducts = useMemo(() => {
    ...
    return safeProducts.filter(p => {
      ...
      const matchesStatus = statusFilter === 'all' || (statusFilter === 'low-stock' ? (p?.stock || 0) < 5 : (p?.stock || 0) > 0);
      ...
      accessor: (p: Product) => (
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{p?.brand || 'Sem Marca'}</p>
          </div>
        </div>
      ),
    },
