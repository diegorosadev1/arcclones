...
export function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After login, the AuthContext will update and the useEffect will handle redirection
      // But we should also check immediately if the user is an admin
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Tempo limite de verificação excedido')), 5000)
      );
