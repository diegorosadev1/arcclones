...
import { useAdminOrdersStore } from "../../orders/store/useAdminOrdersStore";
import { MetricCard } from "@/src/admin/components/shared/MetricCard";

export function Dashboard() {
  const { metrics, salesData, statusDistribution } = useAdminDashboardStore();
  ...
  useEffect(() => {
    useAdminDashboardStore.getState().fetchDashboardData(period);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-zinc-500 text-sm">
            Bem-vindo ao painel administrativo da ARC CLONES.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          label="Ticket Médio"
          value={`R$ ${metrics.ticketAverage.toLocaleString("pt-BR", {
            minimumFractionDigits: 2,
          })}`}
          icon={TrendingUp}
                ...
                >
                  {statusDistribution.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        ["#E11D48", "#3b82f6", "#10b981", "#f59e0b", "#ef4444"][
                          index % 5
                        ]
                      }
                    />
        ...
        <div className="glass-card p-8 space-y-8">
          ...
          <div className="space-y-4">
            {recentOrders.map((order) => (
              <div
                key={order.id}
                className="flex items-center justify-between p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800 hover:border-accent/30 transition-all duration-300"
              >
          ...
          <div className="space-y-4">
            {lowStockProducts.length > 0 ? (
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-zinc-800">
                      <img
                        src={
                          product?.images?.[0] ||
                          "https://picsum.photos/seed/luxury/100/100"
                        }
                        alt={product?.name}
              ...
              <div className="text-center py-12 space-y-4">
                <CheckCircle
                  size={48}
                  className="mx-auto text-emerald-500/20"
                />
                <p className="text-zinc-500 text-sm">
